...
file_path = "/Users/shivavarma/Subratha/frontend/src/App.jsx"

with open(file_path, "r") as f:
    lines = f.readlines()

new_step_1 = """            {orderStep === 1 && (() => {
...
for i, line in enumerate(lines):
    if '{orderStep === 1 && (() => {' in line:
        start_line = i
    if '{orderStep === 2 && (' in line and i > start_line:
        end_line = i
        break

if start_line != -1 and end_line != -1:
    new_content = lines[:start_line] + [new_step_1] + lines[end_line:]
    with open(file_path, "w") as f:
        f.writelines(new_content)
    print("SUCCESS: REPLACED APP.JSX STEP 1")
else:
    print(f"ERROR: Could not find markers mapping. start={start_line}, end={end_line}")
